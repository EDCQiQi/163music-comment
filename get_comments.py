...
def hotcomments(html, songname, i, pages, total, filepath): 
    ...
    if 'hotComments' in html:
        for item in html['hotComments']:
            ...
            user = item['user']
            # 写入文件
            print("热门评论{}: {} ".format(m, item['content']))
...
def comments(html, songname, i, pages, total, filepath): #获取其他评论 20条一页
    ...
    for item in html['comments']:
        # 提取发表评论的用户名
        user = item['user']
        print("全部评论{} : {} ".format(j,  item['content']))
        with open(filepath, 'a', encoding='utf-8') as f:
...
def main():
    ...
    data = {'params': params, 'encSecKey': encSecKey}#模拟加密的数据
    # 获取第一页评论
